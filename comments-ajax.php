...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
        err( __('请填写必填项 （昵称、邮箱、验证码）.') ); // 将 wp_die 改为错误提示   
	elseif ( !is_email($comment_author_email))
        err( __('请填写正确格式Email地址：name@example.com') );// 将 wp_die 改为错误提示   
}

if ( '' == $comment_content )
    err( __('请填写评论内容.') ); // 将 wp_die 改为错误提示   
// 增加: 錯誤提示功能
function err($ErrMsg) {
    header('HTTP/1.1 405 Method Not Allowed');
    echo $ErrMsg;
	...
	<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
		<article id="comment-<?php comment_ID(); ?>" class="comment-body">
			...
			<div class="comment-author vcard">
				...
				<?php #lazyload头像，然而似乎无效了  echo '<img class="avatar" src="https://2heng.xin/wp-content/uploads/2017/08/loadcat.gif" alt="avatar" data-original="' . preg_replace(array('/^.+(src=)(\"|\')/i', '/(\"|\')\sclass=(\"|\').+$/i'), array('', ''), get_avatar( $comment, '40' )) . '" />'; ?>
			</div>
